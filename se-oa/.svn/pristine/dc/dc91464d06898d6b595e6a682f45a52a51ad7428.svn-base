...
public class VideoController extends BaseController {
	...
	private static String IMAGE_EXTESION = "jpg";
	...
	public String del(Long id, Model model) {
		try {
			...
			delParam.put("id", id);
			delParam.put("status", YesORNoEnum.DEL.getCode());
			videoService.update(delParam);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error(e.getMessage(), e);
		}
	...
	public ModelResult<String> upload(MultipartFile file) {
		if (null != file) {
			...
			try {
				...
				video.setTitle(oldFileName.substring(0, oldFileName.indexOf(".")));
				...
				video.setFavoriteCount(random.nextInt(100));
				video.setClickCount(random.nextInt(1000));
	...
	private VideoProps parseVideo(InputStream videoStream, int second) throws IOException {
		...
		try {
			...
			BufferedImage sourceImage = converter.getBufferedImage(frame);
			
			int width = sourceImage.getWidth();
	    	Double scale = Double.valueOf(300)/width;
	    	BufferedImage image = Thumbnails.of(sourceImage).scale(scale).outputQuality(0.8F).asBufferedImage();
